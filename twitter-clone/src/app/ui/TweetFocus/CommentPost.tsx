...
import styles from '@/app/ui/TweetFocus/CommentPost.module.css'
...
export default function CommentPost({ user = "Home", parentId, callback }: { user: string, parentId: string, callback?: (erase?: boolean) => Promise<void> }) {
    ...
    const [replyingTo, setReplying] = useState<string | null>(null);
    const handleImage = () => {
    ...
    useEffect(() => {
        const fetchCoverImage = async () => {
            ...
            if (imageUrl.status == 200 && imageUrl.user?.username) {
                setReplying(imageUrl.user.username)
            }
        };
        ...
        <>
            <div className={styles.BiggerContainer}>
                <div className={styles.ReplyContainer}>
                    <div className={styles.replyFiller}></div>
                    <div className={styles.replyTo} >Replying to&nbsp; <p style={{ color: "#1DA1F2", cursor: "pointer" }} onClick={() => { window.location.href = `/${user}` }} >@{replyingTo}</p></div>
                </div>
                <div className={styles.tweetPostContainer}>
                    ...
                    <form className={styles.tweetContainer} onSubmit={handleSubmit}>
                        <div className={styles.inputContainer}>
                            <textarea ref={text} name="content" placeholder='Post your reply' required></textarea>
